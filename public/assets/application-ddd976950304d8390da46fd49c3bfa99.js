...
$(function() {
  ...
  $('#tb_send_canvas').on('ajax:beforeSend', function(event, xhr, settings) {
    ...
    var htmlLink = $("#html_share_link a").clone().text('SALSA View Link').attr({ 'id': 'salsa_document_view_link' });
    var htmlEditLink = $("#html_share_edit_link a").clone().text('SALSA Edit Link').attr({ 'id': 'salsa_document_edit_link' });
    
    var htmlDiv = $('<div/>').css({ float: 'right' }).append(htmlLink).append(htmlEditLink);
